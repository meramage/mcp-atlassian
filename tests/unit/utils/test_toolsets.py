...
class TestGetEnabledToolsets:
    ...
    @pytest.mark.parametrize(
        "env_value, expected",
        [
            pytest.param(None, DEFAULT_TOOLSETS, id="unset_uses_default"),
            pytest.param("", DEFAULT_TOOLSETS, id="empty_uses_default"),
            pytest.param(" , , ", DEFAULT_TOOLSETS, id="whitespace_uses_default"),
    ...
    def test_all_keyword(self, monkeypatch):
        """Test 'all' keyword returns all 20 toolset names."""
    ...
    def test_all_toolsets_count(self):
        """Verify ALL_TOOLSETS has exactly 20 entries."""
        assert len(ALL_TOOLSETS) == 20
